...
stage1_rm:
    ...
    ; ── E820 memory map (must be called in real mode) ─────────
    call do_e820
...
do_e820:
    ; INT 15h/E820 — store entries at 0x500, count at 0x4F8
    pusha
    xor  ax, ax
    mov  es, ax
    mov  di, 0x500
...
.no_avx:
    ...
    ; Force-write CPU state with absolute address (bypass EDI aliasing)
    mov word [OMEGA_BASE + OMEGA_SZ*1 + 10], OMEGA_CONFIGURED
    ret
...
discover_mem:
    ...
    ; ── Parse E820 map ───────────────────────────────────────
    movzx ecx, word [0x4F8]     ; entry count
    test ecx, ecx
    jz .fallback

    mov esi, 0x500
    xor ebx, ebx               ; total usable bytes (low 32)
.e820_loop:
    push ecx
    cmp dword [esi+16], 1      ; type=1 = usable
    jne .e820_skip
    add ebx, dword [esi+8]     ; length lo
.e820_skip:
    add esi, 24
    pop ecx
    loop .e820_loop
...
.scan_dev:
    ...
    pop eax
    mov [edi+36], eax           ; vendor:device full dword (+36)
    ...
    mov al, ' '
    call pmb
    ; Vendor:Device from [edi+36] dword
    mov eax, [edi+36]
    call pmhex16       ; vendor = low 16
    mov al, ':'
    call pmb
    mov eax, [edi+36]
    shr eax, 16
    call pmhex16       ; device = high 16
    mov esi, msg_cls
    call pmstr
...
.do_selfmod:
    ; OP_SELFMOD: COMPILER node rewrites its own bytecode and re-executes.
    ; arg = node index to mutate (should be NODE_COMPILER = 4)
    ; 
    ; Phase 1: advance COMPILER node state → EXECUTED
    movzx edi, cl               ; arg = node index
    imul  edi, OMEGA_SZ
    add   edi, OMEGA_BASE
    mov   word [edi+10], OMEGA_EXECUTED

    ; Phase 2: overwrite bytecode with "second pass" program
    ; Second pass: PRINT all nodes with updated states, then HALT.
    ; This replaces the entire bytecode in memory (past the magic).
    push esi
    push edi
    push eax

    mov edi, HDGL_LOAD + 2      ; write past magic

    ; PRINT ROOT (0)
    mov byte [edi+0],  0x06 ; OP_PRINT
    mov byte [edi+1],  0x00 ; node 0
    ; PRINT CPU (1)
    mov byte [edi+2],  0x06
    mov byte [edi+3],  0x01
    ; PRINT MEM (2)
    mov byte [edi+4],  0x06
    mov byte [edi+5],  0x02
    ; PRINT COMPILER (4) — now EXECUTED
    mov byte [edi+6],  0x06
    mov byte [edi+7],  0x04
    ; RECURSE IO (3) — advance PCI children one more state
    mov byte [edi+8],  0x04 ; OP_RECURSE
    mov byte [edi+9],  0x03
    ; HALT
    mov byte [edi+10], 0x05
    ...
    ; Phase 3: print self-modification message
    push esi
    mov esi, msg_selfmod
    call pmstr
    pop esi

    ; Phase 4: re-run the bytecode interpreter from the beginning
    ; (ESI will be reset to HDGL_LOAD by the second call)
    push esi
    push ebx
    mov esi, HDGL_LOAD
    mov ebx, [pci_node_idx]     ; restore EBX to post-PCI node count
    ; Re-enter interpreter with new bytecode
    ; ESI already set to HDGL_LOAD; skip magic and loop
    mov esi, HDGL_LOAD + 2      ; point past magic to new instructions
    jmp .loop                   ; re-enter the interpreter loop

    ; (selfmod_no_bc removed - always valid after writing)
...
pmhex16:
    push eax
    push ecx
    rol eax, 16         ; bring low 16 bits to top
    mov ecx, 4
.d: rol eax, 4
    push eax
    and al, 0x0F
...
msg_e820_cnt db "  E820: ",0
msg_e820_ent db " entries",0
msg_e820_tot db "  Total usable: ",0
msg_mem_kb   db "  Conv. (BDA): ",0
...
msg_selfmod  db "  BC: SELFMOD → COMPILER EXECUTED. Bytecode rewritten.",0x0D,0x0A,0

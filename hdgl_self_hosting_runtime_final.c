...
typedef enum { OMEGA_ROOT = 0, OMEGA_CPU, OMEGA_MEM, OMEGA_IO, OMEGA_COMPILER, OMEGA_AST, OMEGA_CODEGEN } OmegaClass;
typedef enum { OMEGA_STATE_INIT = 0, OMEGA_STATE_DISCOVERED, OMEGA_STATE_CONFIGURED, OMEGA_STATE_READY, OMEGA_STATE_EXECUTED, OMEGA_STATE_COMPLETED, OMEGA_STATE_MAX } OmegaState;
...
static inline void hdgl_parse_glyph(HDGLParser* p) {
    ...
    else if (strstr(name, "AST") != NULL) cls = OMEGA_AST;
    else if (strstr(name, "CODEGEN") != NULL) cls = OMEGA_CODEGEN;
    
    Omega* parent = strstr(name, "root") != NULL ? &omega_root : strstr(name, "cpu") != NULL ? &omega_root : strstr(name, "io") != NULL ? &omega_root : strstr(name, "compiler") != NULL ? &omega_root : NULL;
    ...
    if (cls == OMEGA_ROOT) hdgl_print("ROOT"); else if (cls == OMEGA_CPU) hdgl_print("CPU"); else if (cls == OMEGA_MEM) hdgl_print("MEM"); else if (cls == OMEGA_IO) hdgl_print("IO"); else if (cls == OMEGA_COMPILER) hdgl_print("COMPILER"); else hdgl_print("OTHER");
    hdgl_print("]\n");
}
...
static inline void hdgl_parse_branch(HDGLParser* p) {
    ...
    if (strstr(child_name, "compiler") != NULL) child_class = OMEGA_COMPILER;
...
void hdgl_runtime_main(const char* source) {
    ...
    while (p.pos < p.len) {
        ...
        if (p.source[p.pos] == 'r') { 
            if (strstr(&p.source[p.pos], "recurse")) { hdgl_parse_recurse(&p); continue; }
        }
        if (p.source[p.pos] == 'm') { 
    ...
    printf("\nHDGL Execution Complete - %zu glyphs created\n", omega_count);
}
...
int main(int argc, char* argv[]) {
    if (argc > 1) {
        ...
        const char* hdgl_source = "# HDGL Self-Hosting Compiler\nglyph compiler_self\nglyph parser\nglyph codegen\nbranch compiler_self parser\nrecurse compiler_self\nmutate discovered\n";

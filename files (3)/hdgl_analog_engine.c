# Good. Now extend the bootstrap to actually compute Dₙ(r) values
# and run the Kuramoto oscillator. This is the C implementation
# distilled from ll_analog.c to only what HDGL needs.

cat > hdgl_analog_engine.c << 'CEOF'
/*
...
static double strand_omega(int i) {
    /* i = 1..8 (strand A..H) */
    double phi_i = pow(PHI, (double)i);
    return 1.0 / pow(phi_i, 7.0);
}
...
double dn_r(int n, double r, double Omega) {
    ...
    return sqrt(PHI * F * two_n * P * Omega) * pow(r, 1.0);
}
...
/* Compute CV of theta phases (circular std/mean) */
static double compute_cv(Kuramoto8D *s) {
    ...
    /* CV proxy: 1-R ∈ [0,1]; 0=locked, 1=max disorder */
    return 1.0 - R;
}

/* One RK4 Kuramoto step */
static void kuramoto_step(Kuramoto8D *s) {
    ...
    for (int i = 0; i < DIMS; i++) {
        ...
        for (int j = 0; j < DIMS; j++)
            sum += sin(s->theta[j] - s->theta[i]);
        k1[i] = s->omega[i] * (1.0 - g) + K * sum / DIMS;
    }
    /* K2 */
    for (int i = 0; i < DIMS; i++) th_tmp[i] = s->theta[i] + 0.5 * DT * k1[i];
    for (int i = 0; i < DIMS; i++) {
        double sum = 0.0;
        for (int j = 0; j < DIMS; j++)
            sum += sin(th_tmp[j] - th_tmp[i]);
        k2[i] = s->omega[i] * (1.0 - g) + K * sum / DIMS;
    }
    /* K3 */
    for (int i = 0; i < DIMS; i++) th_tmp[i] = s->theta[i] + 0.5 * DT * k2[i];
    for (int i = 0; i < DIMS; i++) {
        double sum = 0.0;
        for (int j = 0; j < DIMS; j++)
            sum += sin(th_tmp[j] - th_tmp[i]);
        k3[i] = s->omega[i] * (1.0 - g) + K * sum / DIMS;
    }
    /* K4 */
    for (int i = 0; i < DIMS; i++) th_tmp[i] = s->theta[i] + DT * k3[i];
    for (int i = 0; i < DIMS; i++) {
        double sum = 0.0;
        for (int j = 0; j < DIMS; j++)
            sum += sin(th_tmp[j] - th_tmp[i]);
        k4[i] = s->omega[i] * (1.0 - g) + K * sum / DIMS;
    }
